    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in quillshield.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .quillshield/ subdirectory too)
  const candidates = [path.join(baseDir, "quillshield.json"), path.join(baseDir, "quillshield.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".quillshield", "quillshield.json"), path.join(baseDir, ".quillshield", "quillshield.jsonc"))
  }
  ...
  // Default to quillshield.json if none exist
  return candidates[0]
}
